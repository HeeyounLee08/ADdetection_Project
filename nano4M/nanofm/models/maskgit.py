...
class MaskGIT(nn.Module):
    ...
    def __init__(
        ...
        self.input_embedding = nn.Embedding(
         ...
         # TODO: Define the input embedding layer
        self.positional_embedding = nn.Parameter(
            torch.randn(seq_len, dim) *self.init_std
        ) # TODO: Define the learnable positional embedding
        self.mask_token = nn.Parameter(
            torch.randn(dim) * self.init_std
        ) # TODO: Define the learnable mask token
    ...
    def generate_random_mask(self, seq: torch.Tensor) -> torch.BoolTensor:
        ...
        mask_counts = torch.randint(1, L+1, (B,), device=seq.device)
        random_vals = torch.rand(B, L, device=seq.device)
        sorted_vals, _ = random_vals.sort(dim=1)
        thresholds = sorted_vals[torch.arange(B), L - mask_counts]
        mask = random_vals >= thresholds.unsqueeze(1)
    ...
    def compute_ce_loss(self, logits: torch.Tensor, target_seq: torch.LongTensor, ignore_index: int = -100) -> torch.Tensor:
        ...
        loss = loss_function(logits.reshape(-1, logits.size(-1)), target_seq.reshape(-1))
    ...
    def generate(
        ...
        for step, k in enumerate(schedule):
            ...
            topk_idx = confidence.topk(k, dim=0).indices
            selected_positions = masked_indices[topk_idx]
            
            # TODO: Get the logits for the `selected_positions`. Shape: [k, vocab_size]
            selected_logits = masked_logits[topk_idx, :]
