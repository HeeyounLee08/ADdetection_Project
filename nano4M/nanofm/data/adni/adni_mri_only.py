...
class AdniLongitudinalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # pad_mask  = np.ones(512, dtype=bool)
        max_len   = 512
        pad_len   = max_len - len(enc_tokens)
        if pad_len > 0:
            enc_tokens = np.concatenate([enc_tokens, np.full(pad_len, PAD_ID)])
            enc_mods   = np.concatenate([enc_mods,   np.full(pad_len,  0)]) 

        positions = np.arange(max_len)
        pad_mask  = enc_tokens != PAD_ID  
